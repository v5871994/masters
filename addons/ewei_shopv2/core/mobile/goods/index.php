...
class Index_EweiShopV2Page extends MobilePage 
{
	...
	public function get_dv_list(){
		...
		$sql='select * from '.tablename('ewei_shop_goods').' where uniacid=:uniacid and pcate=:pcate and status=1 limit '.(($pindex - 1) * $psize) . ',' . $psize;
		$sql_total='select count(*) from '.tablename('ewei_shop_goods').' where uniacid=:uniacid and pcate=:pcate and status=1';
		$params=array(':uniacid'=>$uniacid,':pcate'=>$category);
		$list=pdo_fetchall($sql,$params);
		foreach($list as &$l){
			$l['thumb']=tomedia($l['thumb']);
		}
		$total=pdo_fetchcolumn($sql_total,$params);
