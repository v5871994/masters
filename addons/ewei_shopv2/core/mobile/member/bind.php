...
class Bind_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($member2)) 
			{
				...
				$this->update($member['id'], array('mobile' => $mobile, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'mobileverify' => 1,'is_gobind'=>1));
				unset($_SESSION[$key]);
	...
	//跳过绑定
	public function passbind(){
		global $_W,$_GPC;
		pdo_update('ewei_shop_member',array('is_gobind'=>1),array('uniacid'=>$_W['uniacid'],'openid'=>$_W['openid']));
		show_json(1);
	}
