											...
											foreach ($goods as $index => $good ) 
											{
												if ($good['id'] == $childgoods['gid']) 
												{
													...
													if (empty($showgoods)) 
													{
														...
														$goods_=pdo_fetch('select * from '.tablename('ewei_shop_goods').' where uniacid=:uniacid and id=:id limit 1',array(':uniacid'=>$_W['uniacid'],':id'=>$childgoods['gid']));
														if(($goods_['isdiscount']==1)&&($goods_['isdiscount_time']>0)){
															$item['data'][$childid]['isdiscount_time']=$goods_['isdiscount_time'];
															if($goods_['isdiscount_time']<strtotime('now')){
																$item['data'][$childid]['isdiscount_time']=0;
															}
															$item['data'][$childid]['isdiscount']=1;
														}
													}
												}
											}
