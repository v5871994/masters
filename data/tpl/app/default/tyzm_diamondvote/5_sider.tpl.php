<?php defined('IN_IA') or exit('Access Denied');?><div id="carousel-container" class="carousel slide">
	<ol class="carousel-indicators">
		<?php  $slideNum = 0;?>
		<?php  if(is_array($addata)) { foreach($addata as $row) { ?>
	...
	<div class="carousel-inner" role="listbox"> 
		<?php  if(is_array($addata)) { foreach($addata as $row) { ?>
		<div class="item<?php  if($addata['0'] == $row) { ?> active<?php  } ?>">
			<a href="<?php  if($row['adurl']!='') { ?> <?php  echo $row['adurl'];?> <?php  } else { ?>javascript:;<?php  } ?>">
				<img src="<?php  echo $row['adimg'];?>" title="<?php  echo $row['adtitle'];?>" style="width:100%; vertical-align:middle;">
			</a>
