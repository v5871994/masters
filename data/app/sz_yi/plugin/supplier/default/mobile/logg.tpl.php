...
.log_menu .nav {height:44px; width:33%; border-bottom:1px solid #f1f1f1;background:#fff; border-left:1px solid #f1f1f1; float:left; line-height:44px; font-size:14px; color:#666; text-align:center;}
...
<div class="log_top">
       <div class="title" onclick='history.back()'><i class='fa fa-chevron-left'></i> 提现记录(<span id='total'></span>)</div>
</div>
 
<div class="log_menu">
        <div class="nav navon" style="border-left:0px;width:33%;" data-status=''>全部</div>
        <div class="nav"  data-status='0'>待审核</div>
        <div class="nav"  data-status='1'>已打款</div>
</div>
...
<script id='tpl_log' type='text/html'>
  ...
  <div class="log_list"
       data-applyid='<%log.id%>'>
    <div class="left"><div class='inner'>编号: <%log.applysn%><br><span>
                申请结算金额: <%log.apply_money%> 
                <%if log.status==0%>申请时间:<%/if%>
                <%if log.status==1%>打款时间:<%/if%>
                
                <%log.dealtime%></span></div></div>
   <div class="right">+<%log.apply_money%><br><span><%log.statusstr%></span></div>
</div>
    <%/each%>
</script>
<script id='tpl_empty' type='text/html'>
    <div class="log_no"><i class="fa fa-file-text-o" style="font-size:100px;"></i><br><br><span style="line-height:18px; font-size:16px;">您还没有相关提现记录~</span></div>
</script>
...
function bindScroller(){
        ...
        $(window).scroll(function () {
            ...
            if ($(document).height() <= totalheight) {
                ...
                if (stop == true) {
                    ...
                    page++;
                    core.pjson('supplier/logg', {status:current_status,page: page}, function (morejson) {
                        stop = true;
                        $('#log_loading').remove();
                        $("#container").append(tpl('tpl_log', morejson.result));
                        if (morejson.result.list.length < morejson.result.pagesize) {
                            $("#container").append('<div id="log_loading">已经加载完全部记录</div>');
                            loaded = true;
                            $(window).scroll = null;
                            return;
                        }
        ...
        function getLog(status) {
            ...
            core.pjson('supplier/logg', {status:status, page: page}, function (json) {
                ...
                $('#container').html(tpl('tpl_log', json.result));
                 ...
                 //   var url = core.getUrl('plugin/commission/logg',{'op':'detail','id':applyid});
                 //   location.href = url;
                 //   return;
                 //});
